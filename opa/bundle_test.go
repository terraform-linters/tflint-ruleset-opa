...
func TestFetchBundle(t *testing.T) {
	...
	t.Run("bundle exactly at max size is accepted", func(t *testing.T) {
		// A bundle whose size equals the limit exactly must be accepted; only
		// strictly larger bundles are rejected.
		original := maxBundleSize
		maxBundleSize = int64(len(bundleBytes))
		defer func() { maxBundleSize = original }()
		...
		b, err := fetchBundle(context.Background(), server.URL, "")
		if err != nil {
			t.Fatalf("bundle exactly at the size limit should be accepted, got: %s", err)
		}
		if len(b.Modules) == 0 {
			t.Fatal("expected at least one module")
		}
