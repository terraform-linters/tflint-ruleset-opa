...
func (r *RuleSet) ApplyConfig(body *hclext.BodyContent) error {
	...
	if err != nil {
		...
		if len(ret.Documents) > 0 {
			localData = ret.Documents
		}
	}

	data := mergeData(bundleData, localData)
...
// mergeData deep-merges two data maps. Values in override take precedence.
	...
	for k, v := range override {
		if baseMap, ok := result[k].(map[string]interface{}); ok {
			if overrideMap, ok := v.(map[string]interface{}); ok {
				result[k] = mergeData(baseMap, overrideMap)
				continue
			}
		}
		result[k] = v
