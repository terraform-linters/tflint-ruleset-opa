...
func main() {
	...
	if err := rewriteFileWithNewVersion("main.go", currentVersion, newVersion); err != nil {
		log.Fatal(err)
	}
...
func checkGitStatus() error {
	...
	if !strings.Contains(strings.TrimSpace(stdout.String()), "terraform-linters/tflint-ruleset-opa") {
		return fmt.Errorf("remote.origin is not terraform-linters/tflint-ruleset-opa, got %s", strings.TrimSpace(stdout.String()))
	}
	return nil
}
