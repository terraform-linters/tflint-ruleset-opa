...
test_deny_insecure_ssl_policy_failed if {
	issues := deny_insecure_ssl_policy with terraform.resources as failed_resources

	count(issues) == 1

	issue := issues[_]
	issue.msg == "Insecure SSL policy detected: ELBSecurityPolicy-2016-08"
}
...
test_deny_insecure_ssl_policy_passed if {
	issues := deny_insecure_ssl_policy with terraform.resources as passed_resources

	count(issues) == 0
}

undefined_resources(type, schema, options) := terraform.mock_resources(type, schema, options, {"main.tf": `
...
test_deny_insecure_ssl_policy_undefined if {
	issues := deny_insecure_ssl_policy with terraform.resources as undefined_resources

	count(issues) == 0
}

combined_resources(type, schema, options) := terraform.mock_resources(type, schema, options, {"main.tf": `
resource "aws_lb_listener" "front_end" {
  load_balancer_arn = "arn:aws:elasticloadbalancing:us-east-1:187416307283:loadbalancer/app/my-load-balancer/50dc6c495c6c9a29"
  port              = "443"
  protocol          = "HTTPS"
...
test_deny_insecure_ssl_policy_combined if {
	issues := deny_insecure_ssl_policy with terraform.resources as combined_resources
